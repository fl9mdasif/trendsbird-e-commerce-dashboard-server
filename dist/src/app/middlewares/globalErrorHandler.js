...
const globalErrorHandler = (err, req, res, next) => {
    ...
    if (err.code && err.code.startsWith("P")) {
        if (err.code === "P2002") {
            ...
            if (err.meta && Array.isArray(err.meta.target)) {
                errors = err.meta.target.map((t) => `${t} already exists`);
            }
        }
    ...
    res.status(statusCode).json((0, sendResponse_1.sendError)(message, statusCode, errors.length > 0 ? errors : undefined));
};
